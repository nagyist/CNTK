...
def create_mb_source(image_height, image_width, num_channels, map_file):
    ...
    return C.io.MinibatchSource(C.io.ImageDeserializer(map_file,
        C.io.StreamDefs(
            features=C.io.StreamDef(field='image', transforms=transforms), # first column  in map file is referred to as 'image'
            labels=C.io.StreamDef(field='label', shape=1000))),            # second column is labels and is ignored
        randomize=False)
...
if __name__ == '__main__':
    ...
    plot(load_model(model_file), "graph.png")
